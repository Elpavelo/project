...
# Step 2: Load Data from CSV - updated file name
df = pd.read_csv("employee_performance_updated.csv")
...
# Create new features based on domain knowledge - same as in main_modified.py
...
# Encode the target variable - ensure consistent with main_modified.py
...
# Note: We're excluding employer_name as it's a categorical feature that would need encoding
# In main_modified.py, this is handled as an input but not used for prediction
X = df.drop(columns=["employee_id", "employer_name", "performance", "performance_encoded"])
...
plt.tight_layout()
plt.savefig('output/tuned_feature_importances.png')
...
X_full = df.drop(columns=["employee_id", "employer_name", "performance", "performance_encoded"])
y_full = df["performance_encoded"]
...
# Step 10: Test the pipeline with a sample prediction - to ensure it's compatible with main_modified.py
print("\nTesting pipeline with a sample prediction:")
sample_input = pd.DataFrame({
    'hours_worked': [45.0],
    'projects_completed': [5],
    'attendance_score': [85.0],
    'peer_feedback': [3.0],
    'manager_feedback': [2.0]
})

# Calculate the derived features (same as in training and main_modified.py)
sample_input['productivity_ratio'] = sample_input['projects_completed'] / sample_input['hours_worked']
sample_input['feedback_avg'] = (sample_input['peer_feedback'] + sample_input['manager_feedback']) / 2
sample_input['feedback_diff'] = abs(sample_input['peer_feedback'] - sample_input['manager_feedback'])

prediction = pipeline.predict(sample_input)
prediction_proba = pipeline.predict_proba(sample_input)[0]

performance_label = label_encoder.inverse_transform(prediction)[0]
print(f"Sample prediction: {performance_label}")
print(f"Prediction probabilities: {prediction_proba}")

# Display sample response format as expected by main_modified.py
response = {
